...
def estrai_dati_da_api(query):
    ...
    params = {'q': query, 'maxResults': 11,'printType': 'books', 'inpublisher': '2000', 'langRestrict': 'it', 'orderBy': 'relevance', 'key': api_key}
...
def elabora_dati(items):
    """Trasforma i dati JSON e filtra i libri nell'intervallo 1995-2005."""
    lista_libri = []
    tutte_le_trame = ""
...
def genera_grafico_parole(testo):
    ...
    # Carica le stop words di base
    stop_words = set(stopwords.words('italian'))
    
    # Aggiungi le parole da escludere (inclusa la versione inglese visto che sono termini tecnici)
    ...
    plt.title("Top 10 Parole Chiave nelle Trame: Romance 2000")
    plt.xlabel("Parola")
    plt.ylabel("Frequenza")
    plt.savefig("grafico_parole_romance_2000.png")
    print("Grafico delle parole chiave salvato correttamente!")

if __name__ == "__main__":
    # Avvio della pipeline
    print("Inizio estrazione dati (Romance 2000)...")
    
    dati = estrai_dati_da_api("subject:romance 2000")
    ...
    if dati:
        ...
        df.to_csv("libri_romance_2000.csv", index=False, sep=';', encoding='utf-8-sig')
